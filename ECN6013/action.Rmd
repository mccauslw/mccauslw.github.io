...
* L'agent est averse pour le risque.
...
* Assurance médicale et choix de traitement
...
\[
	-\frac{v''(x)}{v'(x)} = \lambda.
\]
* Plus grand $\lambda$, plus averse; $\lambda = 0$ est la neutralité.
...
\[
  ...
  \end{aligned}
\]
* La prime de risque ici est de $\lambda \sigma^2/2$; on accepte $\lambda \sigma^2/2$ moins en espérance pour avoir un montant certain.
...
* obtient l'utilité $E[v(w)] - e^2/(2a)$, où
    ...
    * $a>0$ est l'habileté de l'agent, {\em observée}
...
\[
	u = E[v(sx + y)] - e^2/(2a) = se + y - s^2 \lambda \sigma^2 - e^2/(2a).
\]
* L'effort optimal :
\[
	\frac{\partial u}{\partial e} = s - e/a,
	\qquad \frac{\partial^2 u}{\partial e^2} = -1/a < 0,
	\qquad
	e^* = sa.
\]
...
\[
	\begin{aligned}
		u^*(s,y) &= s^2a + y - s^2 \lambda \sigma^2 - s^2a^2/(2a) \\
		&= s^2a/2 + y - s^2 \lambda \sigma^2.
	\end{aligned}
\]
* Il accept le contrat $(s,y)$ si $u^*(s,y) > u_0$, ou si
\[
	y + s^2a/2 - s^2\lambda \sigma^2 \geq u_0.
\]
...
\[
	y + s^2a/2 - s^2\lambda \sigma^2 \geq u_0.
\]
* Cette condition est saturée, alors $y=u_0 - s^2a/2 + s^2\lambda \sigma^2$.
* La valeur nette du projet est de
\[
	\begin{aligned}
		\pi = E[x-sx-y] &= (1-s)sa - (u_0 - s^2a/2 + s^2\lambda \sigma^2) \\
		&= sa - s^2a/2 - u_0 - s^2\lambda \sigma^2.
	\end{aligned}
\]
* La part optimale:
\[
	\frac{\partial \pi}{\partial s} = a - sa - 2s\lambda \sigma^2,
	\qquad \frac{\partial^2 \pi}{\partial s^2} = -a-2\lambda\sigma^2 < 0,
\]
\[
	\qquad s^* = \frac{a}{a+2\lambda \sigma^2}.
\]
...
* L'effort efficace égalise le gain marginal (1) et le coût marginal ($e/a$) à $e=a$.
...
* maximise la valeur espérée du projet moins le paiement espéré à l'agent (neutralité pour le risque)
...
* Le principal offre le contrat le plus payant (pour lui).
...
* Intutition : s'il n'avait pas besoin d'inciter $e=1$, il assurerait l'agent complètement.
...
* Les deux contraintes sont saturées, on obtient $s_0^\alpha$ et $s_1^\alpha$ comme la solution deux équations linéaires (les deux contraintes).
