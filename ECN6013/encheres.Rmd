...
* Rappel : quand les valeurs sont privées, l'utilité espérée associée à une enchère de $b_i$, quand la valeur est $v_i$, et
\[
	\Pr[b_i > \max_{j\neq i}(b_j)] (v_i - b_i)
\]
* Pour une loi donnée de $\max_{j\neq i}(b_j)$, on peut maximiser cette utilité comme fonction de $v_i$.
* Passons maintenant au cas des valeurs communes : la valeur $v$ de l'objet est incertaine.
...
\]
où $A$ est une événement; $A^c$, son complément et $X$, une variable aléatoire.
...
* Monopole, discriminant par le prix de 3e degré:
    * plusieurs segments, type observé (sexe, âge, statu étudiant)
...
* Considérer une offre à prendre ou à laisser à $i$ :
    ...
    * Revenu marginal :
		...
		\]
* On appelle $MR_i(v) = v-(1-F_i(v))/f_i(v)$, comme fonction de la valeur aléatoire $v$, le revenu marginal du joueur $i$.
