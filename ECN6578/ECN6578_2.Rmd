...
* Question : Pour quelle valeur de $R$ un rendement net annuel constant de $R$ pendant $k$ ans donne-t-il un rendement net après $k$ périodes de $R_t[k]$?
...
* Asymétrie et aplatissement dans le tableau 1.2
...
$$ z_S = \frac{\hat{S}_{ch}}{\sqrt{6/T}} \sim N(0,1). $$

* Loi asymptotique de l'aplatissement de l'échantillon sous $H_0$:
$$ z_K = \frac{\hat{K}_{ch}-3}{\sqrt{24/T}} \sim N(0,1). $$
...
## Exemple 2: test de l'hypothèse $H_0$ contre $H_1:K > 3$ à un niveau de 1\%
...
$$ \begin{aligned} \mathrm{Cov}[X_1^2, X_2^2] &= E[\mathrm{Cov}[X_1^2,X_2^2|\omega]] + \mathrm{Cov}[E[X_1^2|\omega], E[X_2^2|\omega]] \\ &= \mathrm{Var}[\nu/\omega] \neq 0. \end{aligned} $$
...
## Fonction d'autocorrélation de $r_t$
...
* Avec correction de biais, Ljung-Box (dist. asym.)
...
3. Asymmétrie négative pour des actifs et des indices.
...
8. La fonction d'autocorrélation de $|r_t|$ décroit selon une loi de puissance (plus lentement que de taux exponentiel). (Long mémoire)
...
Trouvez ici les quatres premiers moments non centraux d'une v.a. $N(\mu,\sigma^2)$ :
<https://fr.wikipedia.org/wiki/Loi_normale#Moments>.

1. Le prix d'un actif le 4 janvier est de 12.55 dollars. Le prix de l'actif le 15 fevrier est de 13.05. Quel est le rendement simple annualisé et le log rendement annualisé?

1. On observe un échantillon $X_1,\ldots,X_T$, où $X_t \sim \mathrm{iid}\, N(\mu,\sigma^2)$.
Si on fait les tests 1 et 2 de la diapo "Attention : tests multiples!" quelle est la probabilité d'au moins un rejet, commen fonction de $\alpha$?

## Exercises R

1. Télécharger le fichier des données `d-intc7208.txt` est faire la graphique des rendements avec la commande `plot`.
1. Avec les rendements journaliers de l'action Intel, faites les tests suivant.
Utilisez les commandes ``qnorm`` et ``qchisq`` pour calculer les valeures critiques.
    a. Un test de l'hypothèse que les rendements sont iid gaussiens, avec la statistique test Jarque-Bera.
    a. Un test de l'hypothèse que les rendements sont iid avec variance finie, avec la statistique test Box-Pierce. Utilisez la commande `acf` pour obtenir la fonction d'autocorrélation. Utilisez $m=10$ retards.
