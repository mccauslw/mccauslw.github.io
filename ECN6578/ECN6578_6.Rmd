...
* En faisant des prévisions, on tient compte de l'incertitude sur
    ...
    - les ordres ($p$ et $q$ d'un ARMA(p,q) par exemple) et
    - les modèles (dans une situation où considère plusieurs modèles concurrents).
* Il n'y a pas de recours aux résultats asymptotiques en $T$.

## Éléments de l'analyse bayésienne (dans un modèle sans variables latentes)
...
## Éléments de l'analyse bayésenne (dans un modèle avec variables latentes)
...
* Densités pertinentes :
    * $f(h|\theta)$, la densité des variables d'état (loi de leur évolution)
    * $f(y|\theta,h)$, la densité conditionnelle des données (modèle conditionnel),
    * ${\cal L}(\theta;y^\circ) = f(y^\circ|\theta)$, la vraisemblance,
    * $f(\theta)$, la densité *a priori*,
...
* $f(\theta|y^\circ)$ resprésente notre incertitude sur $\theta$ après qu'on observe $y=y^\circ$.
...
* Considérez le modèle $y_t \sim \mathrm{iid}\, N(\mu, h^{-1})$.
...
## Preuve du résultat de l'exemple 3
