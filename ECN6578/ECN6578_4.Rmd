...
## Prévision à un horizon $h=2$ : 1er perspectif de trois
...
## Prévision à un horizon $h=2$ : 2e perspectif de trois
...
\[
  \mathrm{Var}[E[r_{t+2}|F_{t+1}]|F_t] = \mathrm{Var}[(\phi_1 - \theta_1) a_{t+1}] = (\phi_1 - \theta_1)^2 \sigma_a^2
\]
- Alors,
\[
...
## Prévision à un horizon $h=2$ : 3e perspectif de trois
...
- Convention alternative où l'indice indique le moment où la quantité est connue. : $\mu_{t-1} \equiv E[r_t|F_{t-1}]$.
...
* On suppose que $a_t$ est covariance stationnaire, ce qui entraîne des restrictions.
* Une spécification de $\sigma_t^2$ et non $\mu_t$.
* $\sigma_t$ connu à $t-1$, $\epsilon_t$ à $t$.
...
* Pas assez d'information pour calculer l’effet de levier:
...
* Conclusion: pour un ARCH(1), une asymétrie inconditionnelle ou un effet de levier passe par une asymétrie conditionnelle.
* Même chose pour un ARCH(m).

## Aplatissement

* En général, on ne connait pas le 4ième moment conditionnel.
...
* pas de longue mémoire pour la volatilité
* ne capture pas l'asymétrie inconditionnelle, indépendant de l'asymétrie conditionnelle
* ne capture pas l’effet de levier, indépendant de l'asymétrie conditionnelle
...
où $\sigma_t(h-i) = a_{t+h-i}^2$ si $h-i \leq 0$ ($t+h-i \leq t$)
...
1. Prenez le code de la diapo 'Simulation du modèle ARCH(3)' et modifiez-le pour simuler un GARCH(1,1) gaussien à moyenne zéro pendant $T=1000$ périodes.
Utilisez les valeurs des paramètres $\alpha_0 = 0.000084$, $\alpha_1 = 0.1213$, $\beta_1 = 0.8523$.
Calculez la variance, l'asymétrie et l'aplatissement de l'échantillon.
Suggestion : comparez à la variance, à l'asymétrie et l’aplatissement de la population obtenues ci-haut.
Faites la graphique de $r_t$ et de $\sigma_t^2$. 
...
- Dans Tsay, 3e édition :
    - Sections 1.2.2, 1.2.4, 3.8 intro, 3.8.1
    
- Au site web suivant : https://fr.wikipedia.org/wiki/Maximum_de_vraisemblance
    - Sections Exemple, Principe, Définitions, Propriétés, Exemples
