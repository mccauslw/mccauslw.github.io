...
## Courbe de rendement actuel, il y a un an, il y a deux ans

![Source: gurufocus.com](./YieldCurve.png)
...
*  Remarquez que $y_{1t}$ est affine en $x_t$. On peut écrire
    \[ -p_{1t} = A_1 + B_1 x_t, \]
    où $A_1 = -\beta^2 \sigma^2/2$, $B_1 = 1$.
...
*  On devine (et vérifie plus tard) que dans le modèle de Vasicek,
    ...
    \[ -p_{nt} = A_n + B_n x_t \]
*  On sait déjà que $A_1 = -\beta^2 \sigma^2/2$ et $B_1 = 1$.
*  Rappel :
    \[ x_{t+1} = (1-\phi) \mu + \phi x_t + \xi_{t+1}. \]
...
1. Construction de mesures de volatilité
...
* Sachant $z$, les sommes $r_{At}^\circ$ et $r_{A,t-k}^\circ$ ne se chevauchent pas et
