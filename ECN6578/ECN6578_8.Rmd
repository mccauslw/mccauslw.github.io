...
*  Voici un milieu très simple :
    ...
    *  Il y a $S$ états du monde possibles : $1, \ldots, S$.
    ...
    *  $\pi$ ($S \times 1$), $X$ ($S \times N$) et $q$ ($N \times 1$) sont primitifs.
* Pour un portefeuille $\omega \in \mathbb{R}^N$,
    * $\omega^\top q$ est le prix de $\omega$ dans la première période,
    * $X\omega \in \mathbb{R}^S$ donne sa valeur dans la deuxième période,
    selon l'état.
* Un arbitrage est un portefeuille $\omega \in \mathbb{R}^n$ tel que
...
*  Vecteur $q$ donne les prix à la période 1 :
    \[
    \underset{N\times 1}{q}
    =
    \begin{bmatrix}
...
*  Définition : vecteur $p$ ($S \times 1$) est un vecteur
    des prix d'états si
    \[ X'p = q, \]
    ou, ce qui est équivalent, si pour chaque actif $i$,
...
* Ligne $i$ de cette équation vectorielle, $i = 1,\ldots,N$ :
    \[ 1 = \sum_{s=1}^S G_{si} p_s = \sum_{s=1}^S (1+R_{si}) p_s. \]
...
*  Intuition pour
    ...
    * L'investisseur supporte ce rendement moyen moins élevé
    car l'actif paie plus quand la consommation est plus valorisée.
...
* Comme le CAPM, le CCAPM explique la coupe transversale des moyennes des rendements.
* Il donne aussi une relation entre la consommation et la coupe transversale des rendements.
...
* Ainsi, l'utilité isoélastique est une rationalisation du consommateur représentatif, un seul consommateur dans le modèle.
...
*  Intuition : une grande aversion pour le risque implique
    un très faible volonté à substituer intertemporellement.
...
*  La définition de l'utilité EZ est récursive:
    ...
    \]
* Notez que la restriction $\psi^{-1} = \gamma$ donne $\theta = 1$.
...
* L'approche GMM n'exige, comme modèle, que des conditions de moment inconditionnel comme $E[(1+R_{it})M_t] = 1$.
...
* Le choix de $g(\cdot,\cdot)$ est la spécification d'un modèle.
* Remarquez que l'hypothèse $E[g(w_t,\theta_0)]=0$ est moins forte que la spécification d'une loi paramétrique pour $w_t$.
    * Par exemple $E[(y_i-x_i\beta)x_i] = 0$ versus $y_i = x_i\beta + u_i$, $u_i|x_i \sim N(0,\sigma^2)$.
...
*  Intuition pour GMM :
    ...
    *  $g_T(w,\theta)$ étant un vecteur, il faut choisir une distance
      à zéro.
    *  On peut évaluer le modèle par la proximité minimale de
      $g_T(w,\theta)$ à zéro.
...
*  L'estimation de $\theta$ dépend de $W_T$.
