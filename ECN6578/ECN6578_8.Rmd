...
*  CLM 8.1 FAS
*  CLM 8.2 CCAPM, utilité isoélastique, casse-têtes empiriques
*  CLM 8.4 Utilité Epstein-Zin, utilité non-séparable
*  CLM A.2 GMM

## Le facteur d'actualisation dans un monde sans risque

*  Supposons un monde sans risque, avec taux d'intérêt $R_t$.
*  La valeur actuelle d'un dollar dans un an est de
    \[ M_{t+1}=\frac{1}{1+R_{t+1}}. \]
*  On appelle $M_t$ le facteur d'actualisation.
...
*  Version inconditionnelle (prendre l'espérance
    des deux côtés) :
    \[ E[(1+R_{it})M_t] = 1. \]
*  Avec $\mathrm{cov}[X,Y] = E[XY] - \mu_X\mu_Y$, on obtient
    \[
...
*  maximisation de l'utilité (hypothèse plus forte), et
*  absence de l'arbitrage (moins forte).
...
*  $V$ est séparable en temps,
*  $V$ est séparable en état (utilité espérée)
*  Le taux de préférence de temps ($\delta$) est constant.
*  La maximisation de $V$ sous des contraints
    (richesse, actifs disponible) donne
...
*  Intuition pour
    \[ E[R_{it}-R_{ot}] = -E[1+R_{ot}]\mathrm{cov}[R_{it},M_t] : \]
    Si $\mathrm{cov}[R_{it},M_t] > 0$, $i$ a un rendement
    élevé quand la consommation future est plus valorisée
    (i.e. quand $U'(C_{t+1})$ est élevé). Son prix
    est plus élevé et son rendement moyen est moins élevé.
...
* Pour tester le CCAPM, on peut estimer $\gamma$ et tester
    ...
    $(1+R_i,C)$ est log-normal et homoscédastique.
...
* L'approche GMM n'exige qu'une condition de moment inconditionnel comme $E[(1+R_{it})M_t] = 1$.

## Éléments de GMM

*  Vecteur de variables aléatoires : $w_t\;$ ($J\times 1$)
...
Pour les exercices suivantes, utilisez les log-rendements mensuels des $N=4$ actions des exercices R du cours 7 : IBM, GM, Hershey Foods et Alcoa.
...
1. Effectuer les tests $J_0$, $J_1$, $J_2$ et $J_3$ de la version Sharpe-Lintner du CAPM.
Utilisez le rendement de l'obligation de trésor d'un an comme le rendement $R_f$ sans risque.
1. Effectuer les tests $J_4$ et $J_5$ de la version Black du CAPM. Une condition raisonnable de convergence est que le changement fractionnaire de $\hat{\gamma}^*$ est $10^{-6}$.
