...
* Pour n'importe quel portefeuille $a$ et n'import quel portefeuille $p$ sur la FMV, les coefficients de la régression
  \[
    R_a = \beta_0 + \beta_1 R_{op} + \beta_2 R_p + \epsilon_a
    \quad
    E[\epsilon_a|R_p,R_{op}] = 0
  \]
  sont
  \[
...
## Implications Testables, un actif : Version S-L

* Dans la version S-L, le rendement du marché $R_m$ est le rendement FMV $R_p$, $R_f$ est le rendement zéro-bêta $R_{op}$.
* L'équation $R_a = (1-\beta_{ap}) R_{op} + \beta_{ap} R_p + \epsilon_a$ devient, pour le portefeuille $a$ égale à l'actif $i$,
$R_i = (1-\beta_{im}) R_f + \beta_{im} R_m + \epsilon_i$.
...
## Implications Testables, un actif : Version Black

* Pour la version Black, on a l'implication
\[ E[R_i] = (1-\beta_{im})E[R_{om}] + \beta_{im} E[R_m] \equiv \alpha_{im} + \beta_{im} E[R_m], \]
ou
\[ E[R_i] = \gamma(1-\beta_{im}) + \beta_{im} E[R_m], \]
où
\[ \alpha_{im} = E[R_{om}](1-\beta_{im}) \quad \forall i, \]
\[ \gamma = E[R_{om}]. \]
* Notez bien que la constante $\gamma$ ne dépend pas de $i$, ce qui est une restriction testable.

## Modèle de marché (plusieurs actifs) : Version S-L
...
* L'hypothèse nulle entraînée par le CAPM est $H_0:\alpha=0$

## Modèle de marché (plusieurs actifs) : Version Black
...
* On se sert souvent des rendements mensuels. L'approximation gaussien n'est pas si mal que pour les rendements journaliers.
...
* On peut riposter :
    ...
    * biais de sélection : les actifs des entreprises qui font faillite pendant la période d'observation sont écartés de l'échantillon. Les petites entreprises font faillite plus souvent, et les survivants ont un rendement plus élevé.
...
\]
où
* $f_1,\ldots,f_K$ sont des facteurs (souvent des portefeuilles ou des variables macroéconomiques)
...
* Ross démontre que sous les hypothèses
    * $N$ très grand,
