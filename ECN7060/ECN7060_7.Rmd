...
* Comme $F$, une autre représentation de la loi d'une v.a. qui existe et qui est unique.
* Applications :
...
1. $|\phi_X(t)| \leq E[|e^{itX}|] = 1$.
1. Si $X$ et $Y$ sont indépendants, $\phi_{X+Y}(t) = E[e^{it(X+Y)}] = E[e^{itX}e^{itY}] = E[e^{itX}]E[e^{itY}]$.
1. $\phi(t) = E[\cos tX] + iE[\sin tX]$, par linéarité de l'espérance.
1. Pour $X$ réelle,
    a. $\Re(\phi(t)) = E[\cos tX]$ est paire.
    a. $\Im(\phi(t)) = E[\sin tX]$ est impaire.
1. Pour $X$ symmétrique ($F(-x) = 1-F(x)$ au points de continuité), $\phi$ est rélle.
...
\[
  \phi_X(t) = (1-p) + pe^{it}.
\]
1. Binomial avec valeurs arbitraires
\[
  X = \begin{cases}
...
## Illustration, cas binomiale avec valeurs $x_0$ et $x_1$
...
## Trajet dans le circle unitaire
...
$$ \phi^{(j)}(0) = i^j E[X^j] $$
* Attention : $\phi_X(t) = \exp(-\gamma |t|)$ pour $X$ Cauchy, pas différentiable à $t=0$.
