...
* Si $X$ est une v.a. la fonction caractéristique $\phi_X \colon \mathbb{R} \to \mathbb{C}$ est définie par
\[
  \phi_X(t) \equiv E[e^{itX}].
\]
* Applications :
    * Calcul des moments (mais moins convenable que la fonction génératrice des moments)
    * L'opération de convolution versus l'opération de multiplication, pour la somme des v.a. indépendantes.
    * Utilisée pour prouver le théorème central limite
    * Inférence quand les moments n'existent pas.

## Aparté sur la convolution

* Soit $X$ et $Y$ deux v.a. indépendantes, soit $Z = X + Y$.
* Dénotez $F_x$, $F_y$, $F_z$ les fonctions de répartition de $X$, $Y$, $Z$.
* On peut écrire
\[
  F_z(z) = P(X+Y\leq z) = \int_{\mathbb{R}} F_x(z-y) dF_y(y).
\]
* Si la densité $f_y$ de $Y$ existe,
\[
  F_z(z) = \int_{\mathbb{R}} F_x(z-y) f_y(y) dy.
\]
* Si la densité $f_x$ de $X$ existe aussi,
\[
  f_z(z) = \int_{\mathbb{R}} f_x(z-y) f_y(y) dy \equiv f_x * f_y.
\]
* $f_x * f_y$ est la convolution de $f_x$ et $f_y$.

## Aparté sur les fonctions paires et impaires

* Une fonction $f\colon \mathbb{R} \to \mathbb{R}$ est
    - *paire* si $f(-x) = f(x)$ pour $x \in \mathbb{R}$,
    - *impaire* si $f(-x) = -f(x)$ pour $x \in \mathbb{R}$.
* Soit $g$, $g_1$, $g_2$ des fonctions paires; $h$, $h_1$, $h_2$, impaires.
* $\int_{-a}^a h(x) dx = 0$.
* Fonctions paires :
    - $\cos x$, $x^i$ pour $i$ pair
    - $g_1 + g_2$, $g_1g_2$, $h_1h_2$, $h_1/h_2$, $g \circ h$, $h \circ g$, $h'$.
* Fonctions impaires :
    - $\sin x$, $x^i$ pour $i$ impair
    - $h_1 + h_2$, $gh$, $g/h$, $h/g$, $h_1 \circ h_2$, $g'$
* Pour une fonction $f$ arbitraire, $f = g_f + h_f$ ou
    - $g_f(x) \equiv (f(x) + f(-x))/2$ est paire et
    - $h_f(x) \equiv (f(x) - f(-x))/2$ est impaire.
...
\[
  \phi(t) = \frac{e^{it\theta} - e^{-it\theta}}{2it\theta}
   = \frac{\cos t\theta - \cos t\theta + i\sin t\theta + i\sin t\theta}{2it\theta}
   = \frac{\sin \theta t}{\theta t}.
\]
* Cas spécial, $a=-1$, $b=1$ :
...
```{r berncirc}
plot(Re(phi), Im(phi), type='l', xlim=c(-1,1), ylim=c(-1,1), asp=1)
...
$\frac{d\log \phi(t)}{dt} = \frac{\phi'(t)}{\phi(t)} = -t$
...
La densité de $Y$, en $y$, est :
\[
  f_Y(y) = \frac{1}{\sqrt{2\pi \sigma^2}}
  \exp\left[-\frac{1}{2} \frac{(y-\mu)^2}{\sigma^2}\right].
\]

Pour $\mu=0$, la densité a la même forme fonctionnelle, en $y$, que $\phi_Y(t)$, en $t$.
...
* Résultat : si $E[|X|^k] < \infty$, alors
\[
  \phi^{(j)}(t) = E[(iX)^j e^{itX}], \; 0 \leq j \leq k
\]
...
## La fonction $\mathsf{\theta sinc(\theta t)} = (\sin \theta t) / t$

```{r sinctheta, echo=FALSE}
t = seq(-6*pi, 6*pi, length.out = 200)
plot(t, sin(t)/t, type='l', xaxt='n', yaxt='n')
abline(h=0, v=0)
```

## Théorème d'inversion I

* Le théorème : Soit $\mu$ une mesure borélienne, $\phi(t)$ sa fonction caractéristique. Si $-\infty<a<b<\infty$ et $\mu(\{a\}) = \mu(\{b\}) = 0$,
...
## La notation petit-$o$ de Landau

Soit $f \colon \mathbb{R} \to \mathbb{R}$, $g \colon \mathbb{R} \to \mathbb{R}$.

- $f(x) \in o(g(x))$ veut dire $\lim_{x\to \infty} f(x)/g(x) = 0$.
- Autrement dit :
    - $f(x) = o(g(x))$.
    - $f$ est négligeable devant $g$ asymptotiquement.
    - $g$ est prépondérant devant $f$ asymptotiquement.

Soit $f \colon \mathbb{N} \to \mathbb{R}$, $g \colon \mathbb{N} \to \mathbb{R}$.

- $f(n) \in o(g(n))$ veut dire $\lim_{n\to \infty} f(n)/g(n) = 0$.
- Si $f(n) \in o(n^{-1})$,
    - $\lim_{n\to \infty} nf(n) = 0$,
    - $\lim_{n\to \infty} [nf(n)]^i = 0$, $i \in \mathbb{N}$,
    - $\lim_{n\to \infty} n[f(n)]^i = 0$, $i \in \mathbb{N}$,
    - $[f(n)]^i \in o(n^{-i})$, $[f(n)]^i \in o(n^{-1})$.
