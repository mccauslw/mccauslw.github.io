...
$$ P[L(x) \leq \theta \leq U(x)|x] \quad \mbox{ou} \quad P[l\leq \theta \leq u|x] $$
    * Stratégie symétrique : $L(x)$ et $U(x)$ sont les quantiles $\alpha/2$ et $1-\alpha/2$ de $\theta$, $U(x) - L(x)$ n'est pas forcément minimale
...
$$ 1-\alpha = P_\theta(\chi_{n-1,1-\alpha/2} \leq (n-1)S^2/\sigma^2 \leq \chi_{n-1,\alpha/2}) = P_\theta(C(X)), $$
où
...
* Réponse (deuxième partie) (soit $\alpha_1 = 0.025$, $\alpha_2 = 0.975$)
    ...
    * La solution de $\Phi(\sqrt{n}(t-\mu_U(t))) = \alpha_1 = 0.025$ est celle de
    $$ \sqrt{n}(t-\mu_U(t)) = z_{\alpha_1} = z_{0.025}. $$
    * Alors la solution est $\mu_U(t) = t - z_{\alpha_1}/\sqrt{n}$.
    * La solution de $\Phi(\sqrt{n}(t-\mu_L(t))) = \alpha_2 = 0.975$ est celle de
    $$ \sqrt{n}(t-\mu_L(t)) = -z_{1-\alpha_2} = -z_{0.025}. $$
    * Alors la solution est $\mu_L(t) = t + z_{1-\alpha_2}/\sqrt{n}$.
    * L'intervalle de confiance pour $\alpha_1,\alpha_2$ données est
    \[
      [T-z_{\alpha_1}/\sqrt{n}, T+z_{1-\alpha_2}/\sqrt{n}]
