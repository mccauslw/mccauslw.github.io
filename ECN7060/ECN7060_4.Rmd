...
$$ L \int_a^b X \equiv 
\sup_{n, t_1,\ldots,t_{n-1}}\left\{
\sum_{i=1}^n (t_i-t_{i-1})
\inf_{t\in [t_{i-1},t_i]} X(t) \colon t_0<t_1<\ldots<t_n
\right\} $$

$$ U \int_a^b X \equiv 
\inf_{n,t_1,\ldots,t_{n-1}}\left\{
\sum_{i=1}^n (t_i-t_{i-1})
\sup_{t\in [t_{i-1},t_i]} X(t) \colon t_0<t_1<\ldots<t_n
\right\}, $$
où $t_0 = a$, $t_n = b$.

Notes :

* $L \int_a^b X$ et $U \int_a^b X$ existent toujours, valeurs $\pm\infty$ sont possibles.
* Quand $L \int_a^b X = U \int_a^b X$, l'intégrale riemannienne existe et
\[
  \int_a^b X(\omega) \, d\omega \equiv L \int_a^b X = U \int_a^b X.
\]

##
![](./Figures/Integrales_L_U.png)

## Extensions de l'intégration riemannienne

### Intervalle non-borné

\[
  \int_{0}^\infty X(t)\, dt \equiv \lim_{b\to \infty} \int_0^b X(t)\, dt,
\]

### Une singularité à $a$ et/ou à $b$
...
##
![](./Figures/Deux_cas_limites.pdf)

## Problèmes pour l'intégration riemannienne

* $L \int_0^1 1_\mathbb{Q} = 0$ et $U \int_0^1 1_\mathbb{Q} = 1$, donc l'intégrale riemannienne n'existe pas.

* Soit $\mathbb{Q}_n$ l'ensemble des $n$ premiers rationnels dans $[0,1]$. (L'ordre n'est pas importante.)

* Pour tous $n$, $L \int_0^1 1_{\mathbb{Q}_n} = U \int_0^1 1_{\mathbb{Q}_n} = 0$.

* Notez que
    * $\lim_{n\to \infty} 1_{\mathbb{Q}_n}(t) = 1_\mathbb{Q}(t)$ pour tous $t \in [0,1]$,
    * $0 = \lim_{n\to \infty} U \int_0^1 1_{\mathbb{Q}_n} \neq U \int_0^1 1_\mathbb{Q} = 1$.
    * $1_{\mathbb{Q}_n}(t) \leq 1_{\mathbb{Q}_{n+1}}(t)$ pour tous $t \in [0,1]$,
...
* Chaque cas est de la forme $\sum_{i=1}^n x_i 1_{A_i}(\omega)$.
...
## Formes non-canoniques d'une variable aléatoire simple

* Soit $X$ une variable aléatoire simple.
* Il y a des formes $X = \sum_{i=1}^n x_i 1_{A_i}(\omega)$ non-canoniques.
* Mais $\sum_{i=1}^n x_i P(A_i) = E[X]$ toujours, par l'additivité de $P$.
...
\[
  Y(\omega) = \begin{cases}
    4 & \omega \in B_1 = [0,3/4), \\
    5 & \omega \in B_2 \equiv [3/4,1].
  \end{cases}
\]
...
* Soit $X = \sum_{i=1}^n x_i 1_{A_i}(\omega)$ et $Y = \sum_{i=1}^n y_i 1_{B_i}(\omega)$ des variables aléatoires simples.
* Alors
\[
  \begin{aligned}
  ...
  \end{aligned}
\]
* Même si les expressions pour $X$ et $Y$ sont canoniques, l'expression pour $aX + bY$ ne l'est pas forcément.
...
* Remarques :
    - les $X_n$ ne sont pas forcément simples.
    - pour une variable aléatoire $X \geq 0$ donnée, on peut construire une suite $X_1,X_2,\ldots$ avec les propriétés ci-haut.
...
* Aussi $E[X_n] \geq \sum_i (v_i - \epsilon) P(A_{in})$. (à droite : $E[Y_n]$, $Y_n$ simple)
