...
* 2.2 Construction de $(\Omega, {\cal F}, P)$, possiblement indénombrable, à partir de
    - une semi-algèbre ${\cal J} \subseteq 2^\Omega$ et
    - une $P\colon{\cal J}\to [0,1]$ superadditive et dénombrablement monotone.
* 2.3 Théorème d'extension : pour montrer que ${\cal F}$ est une tribu et que $P$ est une probabilité sur l'espace mesurable $(\Omega, {\cal F})$.
...
* 2.6 Application du théorème pour d'autres $\Omega$
    - suites de tirages à pile ou face $\Omega = \{(r_1,r_2,\ldots): r_i \in \{0,1\}\}$
    - produits cartésien $\Omega = \Omega_1 \times \Omega_2$.

## Théorème d'extension I

Application : spécifier $\Omega$, une semi-algèbre ${\cal J}$, une proto-probabilité $P \colon {\cal J} \to [0,1]$, puis obtenir $(\Omega, {\cal F}, P)$.
...
1. $P(\emptyset) = 0$ et $P(\Omega) = 1$.
1. $P$ est finiement superadditive : pour $A_1,\ldots,A_k \in {\cal J}$ disjoints tels que $\cup_{i=1}^k A_i \in {\cal J}$,
\[
  P\left( \cup_{i=1}^k A_i \right) \geq \sum_{i=1}^k P(A_i).
\]
1. $P$ est dénombrablement monotone : pour chaque suite $A_n \in {\cal J}$ telle que $A \equiv \cup_n A_n \in {\cal J}$, $P(A) \leq \sum_n P(A_n)$.

## Théorème d'extension II

Si les conditions 1-4 tiennent, il y a une tribu ${\cal M}$ sur $\Omega$ et une probabilité $P^*$ sur ${\cal M}$ telles que
...
- $2^\Omega$ est une tribu.
- Pour chaque collection de tribus $\{ {\cal F}_i \colon i \in I\}$ sur $\Omega$, l'intersection $\cap_{i \in I} {\cal F}_i$ est une tribu.
- Pour n'importe quel ensemble ${\cal A} \subseteq 2^\Omega$ de parties de $\Omega$ il y a toujours une seule tribu (dénoté $\sigma({\cal A})$) qui est la tribu la plus petite qui contient ${\cal A}$.

## Deux tribus sur $\Omega = \mathbb{R}$ (Exercice 2.4.5)
...
* Soit ${\cal A}_2$ l'ensemble de tous les intervalles de $\mathbb{R}$.
    * ${\cal A}_2$ est-elle une semi-algèbre?
    * ${\cal A}_2$ plus toutes les réunions dénombrables n'est pas une tribu. L'ensemble de Cantor (pages 16-17) est le complément d'une réunion dénombrable d'intervalles mais n'est pas une réunion dénombrable d'intervalles.
    * Soit ${\cal B} \equiv \sigma({\cal A}_2)$, la tribu la plus petite qui contient ${\cal A}_2$.
    * La spécification directe de $P\colon {\cal A}_2 \to [0,1]$ est ardue.
* Soit ${\cal A}_1 = \{ (-\infty, x] \colon x \in \mathbb{R} \}$.
    * ${\cal A}_1$ est-elle une semi-algèbre?
    * Pourquoi ${\cal A}_1$ est-elle utile?
* Prochaine diapo : démonstration que $\sigma({\cal A}_1) = {\cal B}$.
...
* $\Omega$ est l'ensemble de suites infinies des tirages à pile ou face.
...
## Une proto-probabilité pour $\Omega_1 \times \Omega_2$
...
* Quelques fonctions sur $\Omega$ qui sont des variables aléatoires :
    ...
    * les limites des variables aléatoires:
    ($Z(\omega) \equiv \lim_{n \to \infty} Z_n(\omega)$ pour $\omega \in \Omega)$).
