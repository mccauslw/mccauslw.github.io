...
* Notes :
    ...
    * Il n'y a pas d'asymétrie explicite ici entre $H_0$ et $H_1$.
    * L'asymétrie est une question d'erreur : on favorise le control d'un type d'erreur.

## Tests

* Deux décisions (même notation pour les actions que la semaine passée)
...
* Notes :
    ...
    * attention : quand $W(X)$ est un estimateur de $\theta$, il est facile de confondre une hypothèse avec $R$ ou $R^c$.
...
* Notes :
    ...
    * Avec cette généralité, on peut briser la symétrie entre les deux hypothèses : choisir $c_I \neq c_{II}$.
    * Si la mesure de coût est l'espérance de l'erreur, la spécification du coût se réduit à la choix de $c_I/c_{II}$.
...
* Rappel : ce calcul est une exercice *ex ante*.
...
* Notes :
    * Ce calcul est une exercice *ex post*.
    * L'asymétrie entre $H_0$ et $H_1$ est seulement en termes de $c_{II}/c_I$.

## Lemme Neyman Pearson

### Définition 8.3.5

Pour $\alpha \in [0,1]$, un test avec fonction de puissance $\beta(\theta)$ a un niveau de $\alpha$ si $\sup_{\theta \in \Theta_0} \beta(\theta) \leq \alpha$.

### Définition 8.3.11

Pour $\alpha \in [0,1]$, un test de niveau $\alpha$ avec fonction de puissance $\beta(\theta)$ est uniforment plus puissant si pour chaque $\theta \in \Theta_0^c$ et pour chaque fonction de puissance $\beta'(\theta)$ d'un test de niveau $\alpha$, $\beta(\theta) \geq \beta'(\theta)$.

### Lemme Neyman Pearson

* Supposons que la densité des données est $f(x|\theta_0)$ ou $f(x|\theta_1)$, selon la valeur de $\theta \in \Theta = \{\theta_0, \theta_1\}$.

* On considère des tests de l'hypothèse $H_0\colon \theta=\theta_0$ contre l'hypothèse $H_1\colon \theta=\theta_1$.

* À une condition technique près, un test de niveau $\alpha$ est uniformement plus puissant ssi le test prend la forme $R = \{x \in {\cal X} \colon f(x|\theta_1) > k f(x|\theta_0) \}$ et $P_{\theta_0}[R] = \alpha$.
...
\]
* Si $(\lambda^*,R^*)$ est la solution, $\lambda^*$ est le prix d'ombre de la puissance.
...
## Test du rapport de vraisemblance (LRT, likelihood ratio test)

* La région de rejet d'un test LRT en général, avec $c \in [0,1]$ à spécifier:
$$ R = \left\{x \in {\cal X} \colon \lambda(x) \equiv \frac{\sup_{\theta \in \Theta_0} L(\theta|x)}{\sup_{\theta \in \Theta}L(\theta|x)} \leq c \right\} $$

* Notes :
    * attrait intuitive dans la lumière du lemme Neymann-Pearson
    * réduction de dimension (on spécifie seulement le scalaire $c$)
    * on utilise souvent $-2 \log \lambda$ comme la statistique test à cause de sa loi khi-carré asymptotique.

## Exemple récurrent Bernoulli

* Rappel :
    - $X_1,\ldots,X_n \sim \mathrm{iid}\, \mathrm{Bn}(\theta)$, $\theta \in [0,1]$.
    - $L(\theta|x) = \theta^r (1-\theta)^{(n-r)}$, où $r$ est le nombre de uns.
    - $\hat{\theta}_{\mathrm{EMV}} = \hat{\theta} = r/n$.
    - $r$ est une statistique exhaustive minimale.
* Considérons les hypothèses $H_0:\theta \geq 1/2$ et $H_1:\theta < 1/2$
    - Alors $\Theta_0 = [1/2,1]$,
    - $\Theta = [0,1]$
    - $\Theta_0^c = [0,1/2)$
    
## Exemple Bernoulli (cont.)

* Calculer le rapport des vraisemblances
\[
  \sup_{\theta \in \Theta_0} L(\theta|x) =
  \begin{cases}
    L(\hat{\theta}|x) = \left(\frac{r}{n}\right)^r \left(\frac{n-r}{n}\right)^{n-r}
      & \hat{\theta} \geq 1/2 \\
    L(\frac{1}{2}|x) = \left(\frac{1}{2}\right)^n
      & \hat{\theta} < 1/2
  \end{cases}
\]
\[
  \sup_{\theta \in \Theta} L(\theta|x) = L(\hat{\theta}|x)
  = \left(\frac{r}{n}\right)^r \left(\frac{n-r}{n}\right)^{n-r}
\]
\[
  \lambda(x) = 
  \begin{cases}
  ...
  \end{cases}
\]
* $\lambda(x)$ est une fonction de la statistique exhaustive $r$.
* En général, on peut utiliser $f(t|\theta)$ directement, obtenir le même résultat.
* Les régions de rejet des tests LRT sont
\[
  R_r = \{x \in {\cal X} \colon \sum_{i}x_i \leq r\},\quad r=0,1,2,3,4,5,12.
\]

## Les valeures de $\lambda(x)$ pour $n=12$

|$r$|$\lambda(x)$|$\beta_r(\theta) = P_\theta(R \leq r)$|
|---|--------|--------------------|
|0|0.000244|$(1-\theta)^n$|
...
* Ici, le rapport de vraisemblance est
\[
  \lambda(x) = \frac{L(\tfrac{1}{2})}{L(\hat{\theta})}
  = \frac{(\tfrac{1}{2})^n}{\left(\frac{r}{n}\right)^r \left(\frac{n-r}{n}\right)^{n-r}}
  = \frac{(n/2)^n}{r^r(n-r)^{n-r}}.
\]
...
## Un test bayésien de $H_0:\theta \geq \tfrac{1}{2}$ contre $H_1:\theta < \tfrac{1}{2}$.

* Soit $n=12$, comme avant.
* Soit $\theta \sim \mathrm{Be}(\alpha, \beta)$, où $\alpha=1$, $\beta=1$ (loi uniforme).
* La loi *a posteriori* est $\theta|x \sim \mathrm{Be}(\alpha+r, \beta+n-r)$.
* Mettons qu'on observe $r=4$.
* La loi *a posteriori* est $\theta|x \sim \mathrm{Be}(5, 9)$ (prochaine diapo).
* Aparté : considérons la possibilité d'arrêter ici.
...
ou si
\[
  \frac{c_I}{c_{II}} \geq `r pbeta(0.5, 5, 9)/(1-pbeta(0.5, 5, 9))`.
\]

## La densité *a posteriori* $\mathrm{Be}(5,9)$

```{r denpost}
theta = seq(0, 1, length.out = 1000)
plot(theta, dbeta(theta, 5, 9), type = 'l')
```
...
Un modèle composé, où
le modèle $M \in \{H_0 : \theta=\tfrac{1}{2},H_1 : \theta\neq\tfrac{1}{2}\}$,
le paramètre $\theta$ et les données sont aléatoires :
\[
  \begin{aligned}
...
\[
  \frac{\Pr[M=H_0|x]}{\Pr[M=H_1|x]} = \frac{\Pr[M=H_0]}{\Pr[M=H_1]}
  \cdot \frac{f_0(x)}{f_1(x)},
\]
où
    - Le premier facteur est le rapport des cotes *a priori*
    - Le deuxième facteur est le facteur de Bayes, le ratio de *vraisemblances marginales*
    - La vraisemblance marginal $f_0(x)$ est un genre de score de succès de prévision hors échantillon du modèle $H_0$ où $\theta = \tfrac{1}{2}$
    - $f_1(x)$ est celui du modèle $H_1$.

## La décision optimale

Pour la loi *a priori* $\mathrm{Be}(\alpha, \beta)$, $\alpha = \beta = 1$, $r=4$ et
$n=12$,
\[
  f_0(x) = `r 0.5^12`,
\]
\[
  \quad f_1(x) = `r exp(lbeta(5, 9) - lbeta(1, 1))`,
\]
\[
  \quad \frac{f_0(x)}{f_1(x)} = `r exp(12 * log(0.5) + lbeta(1, 1) - lbeta(5,9))`
\]
