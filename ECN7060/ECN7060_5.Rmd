...
* Preuve :
    ...
    * Alors par l'inégalité de Markov,
    ...
    \]
* Notes :
    * $\mathrm{Var}[Y] = \infty$ est possible, mais l'inégalité ne contraint pas.
...
Soit $Y = Z = 0$, $Y_n$, $Z_n$ des suites de v.a. telles que
...
* $\sum_n \Pr[Y_n \neq Y] = \sum_{1/n^2} = \pi^2/6 < \infty$ alors $Y_n \overset{a.s.}{\to} Y$.

Pour la suite $Z_n$ :

* $\Pr[Z_n \neq Z] = 1/n \to 0$ alors $Z_n \overset{p}{\to} Z$.
* Mais $\sum_n \Pr[|Z_n - Z| \geq 1] = \sum_n 1/n = \infty$.
* Si $Z_n = 1_{(0,1/n)}$, $P$ est la mesure de Lebesgue sur $\Omega = [0,1]$,
...
* Par la convexité de $\phi$, il y une fonction $g \colon \mathbb{R} \to \mathbb{R}$ telle que
    ...
    * $g(x) \leq \phi(x)$
    * $g(E[X]) = \phi(E[X])$
* Notez que $\phi$ n'a pas forcément une dérivée à $E[X]$, auquel cas $g$ n'est pas unique.
...
* Monotonicity de $F$ par monotonicité de probabilité.
