...
* Supposons que
    * $\lim_{t \downarrow 0}X_t(\omega) = X(\omega)$, $\omega \in \Omega$, et
...
## Exemple, suites de tirages au pile ou face

* Soit $\Omega = \{(r_1,r_2,\ldots) \colon r_i \in \{P,F\}\}$.
* Soit ${\cal F}$, $P$ l'extension de ${\cal J}$ et $P$ de la Section 2.6 de Rosenthal.
* Rappel: la probabilité de l'histoire $A_{a_1a_2\ldots a_n}$ est $2^{-n}$.
* Soit
\[
  X_n = \begin{cases} 1 & r_n = F, \\ 0 & r_n = P. \end{cases}
\]
* Soit $S_n = \frac{1}{n} (X_1 + \ldots + X_n)$.
* Soit $t_n = \sqrt{n}(S_n - \frac{1}{2})$.

## Exemple, continué

```{r clt, eval=FALSE, include=FALSE}
n_4 = seq(0,4); n_8 = seq(0,8); n_16 = seq(0,16)
tvals_4 = sqrt(4) * (n_4 - 0.5*4)
pvals_4 = dbinom(n_4, 4, 0.5)
plot(tvals_4, pvals_4, 'p')
```
