...
Now let $X = (X_1,\ldots,X_n)$, a random vector of $n$ coin tosses. Then for each vector $x=(x_1,\ldots,x_n)$, a possible outcome of a sequence of $n$ coin tosses,
$$ P_\theta[X=x] = \prod_{i=1}^n \theta^{x_i} (1-\theta)^{1-x_i} = \theta^{n_1}(1-\theta)^{n_0}, $$
where $n_1$ is the number of times $X_i = 1$ is observed and $n_0$ is the number of times $X_i = 0$ is observed.
...
As such, it gives the probabilities of all possible realizations of $n$ coin tosses; some values of $x$ are more probable than others, for the given value of $\theta$.
2. Alternatively, we can think of it as a function of $\theta$ for a fixed value $x$ of a sequence of $n$ coin tosses. It makes sense to think of the value of $x$ as arising from observing the result of a single experiment featuring $n$ coin tosses. We call this function a *likelihood function* and denote it ${\cal L}(\theta; x) = \theta^{n_1}(1-\theta)^{n_0}$.
...
The likelihood function says how probable the realized sample was, for each value $\theta$. In a sense, ${\cal L}(\theta;x)$ is a record of how well the value $\theta$ "predicted" the realized sample $x$.
...
1. Multivariate densities naturally take the form of products of densities. Taking the logarithm of a product yields a sum of logarithms, which is easier to work with; for example, it is often easier to take derivatives.
...
1. Several theoretical results related to maximum likelihood, as a procedure, involve first and second derivatives of the log likelihood function.
...
Frequentist, as opposed to Bayesian, analysis is intrinsically *ex ante*, which means that it focusses on the behaviour of some decision rule---here the choice of $\hat\theta$ as a plausible value of $\theta$---in repeated samples, where each sample gives a different realization of the random vector $X$.
The term ex ante refers to the fact that you can do this analysis before you observe the data, the realization of $x$.
...
We call it an *estimator* (FR: estimateur) of the parameter $\theta$, emphasizing the *procedure* rather than the result.
...
We now graph the log likelihood function ${\cal L}(\theta;x)$ and the score function $s(\theta;x)$ for every realization of the experiment.
```{r LSH_low_n}
...
> Exercise: proof the identity $$\sum_{i=1}^n (x_i-\mu)^2 = n(\bar x - \mu)^2 + \sum_{i=1}^n (x_i - \bar x)^2.$$

Dividing the identity by $n$ gives a sample analogue of the following population identity, which is also helpful to know. Its proof is very similar.
\[
  E[(X-a)^2] = E[(X-E[X])^2] + (E[X] - a)^2 = \mathrm{Var}[X] + (E[X] - a)^2.
\]

Now we can write the joint density of $f(x)$ as
\[
  f(x) = \prod_{i=1}^n \frac{1}{\sqrt{2\pi\sigma^2}}
\exp\left[-\frac{1}{2\sigma^2}(x_i-\mu)^2\right]
